...
def get_gpu_compute_capability():
    """Get GPU compute capability to determine Flash Attention support"""
...
def supports_flash_attention_2():
    """Check if current GPU supports Flash Attention 2 (requires compute capability >= 8.0)"""
    compute = get_gpu_compute_capability()
    supports = compute >= 8.0
    
    if torch.cuda.is_available():
        gpu_name = torch.cuda.get_device_name(0)
        logger.info(f"GPU: {gpu_name}")
        logger.info(f"Compute Capability: {compute}")
        logger.info(f"Flash Attention 2 {'supported' if supports else 'NOT supported (requires >= 8.0)'}")
...
def main():
    ...
    parser.add_argument('--use_flash_attention', action='store_true', help='Force enable Flash Attention 2 (may fail on older GPUs)')
    parser.add_argument('--no_flash_attention', action='store_true', help='Disable Flash Attention 2 (use eager attention)')
    ...
    if args.use_flash_attention:
        use_flash_attn = True
        logger.info("Flash Attention 2 enabled (forced by --use_flash_attention)")
    elif args.no_flash_attention:
        use_flash_attn = False
        logger.info("Flash Attention 2 disabled (forced by --no_flash_attention)")
    else:
        # Auto-detect based on GPU capability
        use_flash_attn = supports_flash_attention_2()
        if not use_flash_attn:
            logger.warning("Flash Attention 2 disabled - GPU does not support it (using eager attention)")
            logger.warning("Training will be slower but should work on T4 GPUs")
    ...
    # Only add attn_implementation if using Flash Attention
    if use_flash_attn:
        model_kwargs["attn_implementation"] = "flash_attention_2"
        logger.info("Loading model with Flash Attention 2")
    else:
        logger.info("Loading model with eager attention (standard PyTorch)")
